...
def main():
    ...
    for name, p in model.named_parameters():
        if p.requires_grad:
            assert "lora" not in name.lower() or ("A" in name or "B" in name), f"Parâmetro inesperado com grad: {name}"
    ...
    config = build_run_config(
        ...
        device=device,
        run_dir=run_dir,
        extra_fields={"rank": rank, "alpha": alpha},
    )
    ...
    delta_map = build_lora_delta_map(model, rank, alpha)
